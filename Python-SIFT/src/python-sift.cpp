...
float SIFT::totalSumO( Mat src ) //¼ÆËãËùÓÐÔªËØµÄ×ÜºÍ
{
	int k=src.rows;
	int m=src.cols;
	float num=0;
...
cv::Mat SIFT::search( Mat src,Mat dst ) //°ÑdstÖÐÎª1µÄsrcÖÐ¶ÔÓ¦ÐÐÕÒ³öÀ´£¬ ??
{
	int num=0;
...
cv::Mat SIFT::reshapeX( Mat I ) //±äÎª?Î¬¾ØÕó
{
	CvSize size=I.size();
...
cv::Mat SIFT::reshapeY( Mat I ) //±äÎªnÎ¬¾ØÕó
{
	CvSize size=I.size();
...
cv::Mat SIFT::square( Mat src ) //¶ÔÃ¿¸öÔªËØÇóÆ½ ?
{
	int r=src.rows;
	int c=src.cols;
	Mat X(r,c,CV_32F);
...
cv::Mat SIFT::sp_find_sift_grid( Mat I,Mat gridX,Mat gridY,int patchSize, float sigma_edge )
{
	...
	//´Ë´¦ÐèÒªÅÐ¶Ï×Ü¹²´«Èë¶àÉÙ¸ö±äÁ¿£¬Èç¹û±äÁ¿ÊýÐ¡ÓÚ5£¬¾Í°Ñsigma_edgeÉèÖÃÎª1
	float angle_step=2*pi/num_angles;

	//³õÊ¼»¯angles ÎªÒ»¸öÒ»Î¬¾ØÕó´Ó0µ½2*pi£»¼ä¸ôÎªangle_step
	Mat angles=create(0,2*pi,angle_step);
	angles=deleteO(angles); //É¾³ý×îºóÒ»¸öÔªËØ

	CvSize size=I.size();
	//int hgt=size.height;
	//int wid=size.width;

	int num_patches=gridX.total();//¼ÆËãgridX×Ü¹²ÓÐ¶àÉÙ¸öÔªËØ
	...
	I_X=filter2(GX,I);              //ÒòÎªI£¬Í¼Æ¬¶ÁÈë²»Í¬£¬ËùÒÔI_X²»Í¬£¬ÓëIÓÐ¹ØµÄ¾ù²¼ÏàÍ¬£¬µ«ÊÇ£¬¶¼ÕýÈ·
	...
	sample_x=reshapeX(sample_x);//±äÎªÒ»¸ö1Î¬¾ØÕó
...
/*		float *ptc2=gridY.ptr<float>(c);*/
		...
		int num_pix = sample_px.total();//¼ÆËãsample_pxÔªËØ×ÜÊý
...
cv::Mat SIFT::sp_normalize_sift_arr( Mat sift_arr,float threshold )
{
	...
	sift_arr_hcontrast=sift_arr_hcontrast/repmat(search(siftlen,normalize_ind1),1,sift_arr.cols);//´Ë´¦Ó¦Îªµã³ý
