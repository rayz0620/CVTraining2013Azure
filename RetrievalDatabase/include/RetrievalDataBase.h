...
public:
	RetrievalDatabase();
	RetrievalDatabase(const path& dbPath, const string& extension = ".gz", bool isRecursive = true );
	//bool convertFromRaw(const path& rawPath, const path& targetPath, const string& extension = ".gz", bool isRecursive = true);
	bool loadraw(const path& dbPath, const string& extension = ".gz", bool isRecursive = true  );
	bool load(const path& dbPath, const string& extension = ".gz", bool isRecursive = true  );
	bool save(path targetPath, const string& extension = ".gz");
	~RetrievalDatabase();
private:
};
